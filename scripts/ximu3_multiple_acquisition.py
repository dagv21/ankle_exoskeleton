...
class Connection:
    def __init__(self, connection_info):
        ...
        self.pub_imu_foot = rospy.Publisher('/foot/imu_data/', IMUData, queue_size=2)
        self.pub_imu_shank = rospy.Publisher('/shank/imu_data/', IMUData, queue_size=2)
        self.pub_q_foot = rospy.Publisher('/foot/quaternion', IMUData, queue_size=2)
        self.pub_q_shank = rospy.Publisher('/shank/quaternion', IMUData, queue_size=2)
    ...
    def __inertial_callback(self, message):
        msg = IMUData()
        msg.gyro_x = message.gyroscope_x
        msg.gyro_y = message.gyroscope_y
        msg.gyro_z = message.gyroscope_z
        msg.accel_x = message.accelerometer_x
        msg.accel_y = message.accelerometer_y
        msg.accel_z = message.accelerometer_z
        if "foot" in self.__prefix:
            self.pub_imu_foot.publish(msg)
        elif "shank" in self.__prefix:
            self.pub_imu_shank.publish(msg)
    ...
    def __quaternion_callback(self, message):
        msg = IMUData()
        msg.quat_x = message.w
        msg.quat_y = message.x
        msg.quat_z = message.y
        msg.quat_w = message.z
        if "foot" in self.__prefix:
            self.pub_q_foot.publish(msg)
        elif "shank" in self.__prefix:
            self.pub_q_shank.publish(msg)
...
def main():
    try:
        ...
        #     connection.send_command("strobe")  # example command with null value
            ...
            connection.send_command("ahrs_message_type", 0) # 0:Quaternion   2:Euler Angles
