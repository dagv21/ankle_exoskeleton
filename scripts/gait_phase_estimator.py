...
class GaitPhaseEstimator:
    ...
    def train_model(self):
        ...
        y_train = np.array(gait_progress).reshape(-1, 1)
        X_train_Derivative = np.array(adjusted_force_derivatives).reshape(-1, 1)
        a_train = np.array(adjusted_angle).reshape(-1, 1)
        a_train_Derivative = np.array(adjusted_angle_derivatives).reshape(-1, 1)

        #print("Force data train shape:", X_train.shape)
        #print("Force derivative data train shape:", X_train_Derivative.shape)
        #print("Angle data train shape:", a_train.shape)
        #print("Angle derivative data train shape:", a_train_Derivative.shape)

        #print("Force data train shape:", y_train.shape)
        ...
        #print("X_combined_reshaped_train shape:", X_combined_scaled_train.shape)
    ...
    def estimate_phase(self):
        if ((self.modelLoaded is not None) and (self.angleUpdated == True) and (self.forceUpdated == True)):
            ...
            if len(self.data_sequence) == self.samples_size:
                ...
                #print("x_combined_data shape: ", x_combined_data.shape)
                
                # Perform prediction
                estimated_phase = self.model.predict(x_combined_data, verbose=0).flatten()[0]
    ...
    def run(self):
        rate = rospy.Rate(100)  # Set the frequency to 200 Hz
